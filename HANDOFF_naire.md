...
| `alpha` (英大文字 Excel 列名) | A=0, B=1, ..., Z=25, AA=26, ..., ZZ=701, AAA=702, ... | `A, B, ..., Z, AA, ..., ZZ, AAA, ...` |

**桁指定（`?` の数）の意味**: 最小幅。指定桁未満は埋め文字で先頭埋め。
- `decimal` / `hex` は `0` で埋める
- `alpha` は bijective base-26 のため**パディングを行わない**。`?` `??` `???` `????` のどれを使っても、出力幅は値そのものに対応する自然な文字数になる（値 0 → `A`、値 26 → `AA`、値 702 → `AAA`）

桁を超える値は自然に拡張される（例: `??` で値 100 → `100`）。
